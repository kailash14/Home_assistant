...
const PATIENTS = [
  ...
  },
  {
    ...
    notes:
      "Mr. Rajesh Sharma, 55M, post-CABG day 18. Surgical wound healing well, no signs of infection. Mild chest discomfort on exertion which subsides with rest. Walking 15 mins daily as advised. Compliance with medications good. Sleep disturbed due to anxiety about returning to work.",
  },
  {
    id: "PT-0592",
    name: "Fatima Begum",
    ...
    notes:
      "Mrs. Fatima Begum, 78F, COPD Stage III on home O2 at 2L/min. SpO2 on O2 is 89%. Increased breathlessness since yesterday. Sputum yellowish-green since morning. Son reports she refuses O2 cannula at night.",
  },
];
...
function vitalStatus(key, v) {
  ...
  if (key === "bp_dia")
    return val >= 110
      ...
      : "normal";
  if (key === "spo2")
    return val < 88 ? "critical" : val < 92 ? "elevated" : "normal";
  if (key === "pulse")
    return val > 120 || val < 40
      ? "critical"
      : val > 100 || val < 60
      ? "elevated"
      : "normal";
  if (key === "temp")
    return val >= 102 || val <= 95
      ? "critical"
      : val >= 99.5
      ? "elevated"
      : "normal";
...
const generateMockAnalysis = (patient, vitals, notes) => ({
  patient_summary: `${patient.age}${
    patient.gender
  } patient with recorded BP of ${vitals.bp_sys || "--"}/${
    vitals.bp_dia || "--"
  } mmHg, Fasting Glucose ${vitals.glucose_fasting || "--"} mg/dL, and Weight ${
    vitals.weight || "--"
  } kg. Reviewing notes for further intervention.`,
  extracted_entities: {
    symptoms: [
      { symptom: "Reported in notes", duration: "Recent", severity: "medium" },
    ],
    medications_current: [],
    medications_stopped: [],
  },
  ...
  vitals_assessment: [
    {
      ...
      value: `${vitals.bp_sys || "--"}/${vitals.bp_dia || "--"} mmHg`,
      status: vitalStatus("bp_sys", vitals.bp_sys),
      note: "Updated from input",
    },
    {
      vital: "Fasting Glucose",
      value: `${vitals.glucose_fasting || "--"} mg/dL`,
      status: vitalStatus("glucose_fasting", vitals.glucose_fasting),
      note: "Updated from input",
    },
    {
      vital: "Weight",
      value: `${vitals.weight || "--"} kg`,
      status: vitalStatus("weight", vitals.weight),
      note: "Updated from input",
    },
  ],
  care_actions: [
    {
      priority: "today",
      action: "Review updated vitals and notes.",
      owner: "nurse",
    },
  ],
  escalation: {
    needed: vitalStatus("bp_sys", vitals.bp_sys) === "critical",
    urgency: "routine",
    reason: "Standard check or abnormal vital recorded.",
...
const generateMockHandoff = (patient, vitals) => ({
  sbar: {
    situation: `Patient ${patient.name} has updated vitals. BP: ${
      vitals.bp_sys || "--"
    }/${vitals.bp_dia || "--"}.`,
  ...
  critical_alerts:
    vitalStatus("bp_sys", vitals.bp_sys) === "critical"
      ? [`Critical BP: ${vitals.bp_sys}/${vitals.bp_dia}`]
      : [],
...
function buildCarePlanText(patient, plan) {
  const goals = (plan.goals || [])
    .map((g) => `• ${g.goal}: ${g.target} (${g.timeline})`)
    .join("\n");
  const schedule = (plan.daily_schedule || [])
    .map(
      (b) =>
        `*${b.day}*\n${(b.tasks || [])
          .map((t) => `  ${t.time}: ${t.task} [${t.owner}]`)
          .join("\n")}`
    )
    .join("\n\n");
...
function VitalInput({ label, value, onChange, unit, statusKey }) {
  ...
  const borderColor =
    normalizeStatus(st) === "critical"
      ? COLORS.critical
      : normalizeStatus(st) === "elevated"
      ? COLORS.high
      : normalizeStatus(st) === "low"
      ? "#0369A1"
      : COLORS.border;
  return (
    <div
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <button
            onClick={handleSend}
            disabled={!selectedNurse || sending}
            style={{
              flex: 1,
              padding: 13,
              borderRadius: 10,
              border: "none",
              cursor: selectedNurse && !sending ? "pointer" : "not-allowed",
              background:
                selectedNurse && !sending ? COLORS.primary : "#CBD5E1",
              color: "white",
              fontWeight: 700,
            }}
          >
...
function App() {
  ...
  const updateVital = (key, val) => {
    setVitalsByPatient((p) => ({
      ...p,
      [selectedPatientId]: {
        ...(p[selectedPatientId] || EMPTY_VITALS),
        [key]: val,
      },
    }));
  };
  ...
  const loadDemoData = () => {
    ...
    setVitalsByPatient((p) => ({
      ...p,
      [selectedPatientId]: { ...patient.vitals },
    }));
  ...
  const getPatientContextStr = () => `
    Patient: ${selectedPatient.name}, ${selectedPatient.age}${
  ...
  const runAnalysis = async () => {
    if (analysis)
      return setAnalysisByPatient((p) => ({ ...p, [selectedPatientId]: null }));
    setErrorMessage("");
    ...
    if (!apiKey) {
      ...
      addLog({
        actionName: "Analyze Visit (Dynamic Mock)",
        status: "success",
        durationMs: 15,
        request: { source: "local" },
        response: "Loaded Mock Analysis",
      });
      return;
    }
    ...
    try {
      ...
      addLog({
        actionName: "Analyze Visit (Claude API)",
        status: "success",
      ...
      addLog({
        actionName: "Analyze Visit (Failed)",
        status: "error",
        durationMs: 0,
        request: {},
  ...
  const generateCarePlan = async () => {
    if (carePlan)
      return setCarePlanByPatient((p) => ({ ...p, [selectedPatientId]: null }));
    setErrorMessage("");
  ...
  const generateHandoff = async () => {
    if (handoff)
      return setHandoffByPatient((p) => ({ ...p, [selectedPatientId]: null }));
    setErrorMessage("");
  ...
  const handleHandoffConfirm = (nurse) => {
    ...
    addLog({
      actionName: "Shift Handoff Assigned",
      status: "success",
      durationMs: 0,
          ...
          {["copilot", "api"].map((t) => (
            <button
              ...
              style={{
                ...
                cursor: "pointer",
                fontWeight: 700,
                background: activeTab === t ? COLORS.primary : "#E2E8F0",
                color: activeTab === t ? "white" : "#1E293B",
                fontSize: 13,
              }}
            >
        ...
        <main style={{ flex: 1, padding: 18, overflow: "auto" }}>
          {activeTab === "copilot" ? (
            <>
                ...
                >
                  ...
                  <div style={{ fontSize: 13 }}>
                    <div style={{ marginBottom: 6 }}>
                      <strong>Nurse:</strong>{" "}
                      {assignedNurses[selectedPatient.id] ||
                        selectedPatient.nurse}
                    </div>
                    <div style={{ marginBottom: 6 }}>
              ...
              <Card title="Vitals — Current Visit" style={{ marginBottom: 14 }}>
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                  <VitalInput
                    label="BP Systolic"
                    value={vitals.bp_sys}
                    onChange={(v) => updateVital("bp_sys", v)}
                    unit="mmHg"
                    statusKey="bp_sys"
                  />
                  <VitalInput
                    label="BP Diastolic"
                    value={vitals.bp_dia}
                    onChange={(v) => updateVital("bp_dia", v)}
                    unit="mmHg"
                    statusKey="bp_dia"
                  />
                  <VitalInput
                    label="Pulse"
                    value={vitals.pulse}
                    onChange={(v) => updateVital("pulse", v)}
                    unit="bpm"
                    statusKey="pulse"
                  />
                  <VitalInput
                  ...
                  <VitalInput
                    ...
                    value={vitals.glucose_fasting}
                    onChange={(v) => updateVital("glucose_fasting", v)}
                    unit="mg/dL"
                    statusKey="glucose_fasting"
                  />
                  <VitalInput
                    label="Weight"
                    value={vitals.weight}
              ...
              {/* NURSE NOTES */}
              <Card
                title="Nurse Notes — Current Visit"
              ...
              >
                <textarea
                  ...
                  rows={4}
                  placeholder="Enter observations, symptoms, complaints..."
                  style={{
                    width: "100%",
                    border: `1px solid ${COLORS.border}`,
                    borderRadius: 10,
              ...
              {analysis && (
                <Card
                  title="Visit Intelligence Analysis"
                  style={{ marginBottom: 14 }}
                >
                  <div
                    style={{
                  ...
                  >
                    <div>
                      ...
                      {(analysis.extracted_entities?.symptoms || []).map(
                        (item, i) => (
                          <div
                            key={i}
                            style={{
              ...
              {carePlan && (
                ...
                >
                  <div
                    style={{ fontSize: 17, fontWeight: 800, marginBottom: 12 }}
                  >
              ...
              {handoff && (
                <Card title="Shift Handoff (SBAR)" style={{ marginBottom: 14 }}>
                  ...
                  {(handoff.critical_alerts || []).map((a, i) => (
                    <div
                      key={i}
                      style={{
                        fontSize: 13,
                  ...
                  {handoffConfirmation &&
                    handoffConfirmation.patientId === selectedPatientId && (
                      <div
                        style={{
                          background: "#F0FDF4",
            ...
            <Card title="CareIQ API Reference & Settings">
              <div style={{ marginBottom: 20 }}>
                ...
                >
                  Leave empty to use dynamic demo mode. Add a key to generate
                  responses using real Claude API.
                </p>
              </div>
              ...
              <div style={{ marginBottom: 20 }}>
                ...
                >
                  <strong>Check this ON if deployed to Netlify</strong> (with
                  the <code>_redirects</code> file configured). <br />
                  <strong>
                    Keep this OFF if testing locally or in sandbox
                  </strong>{" "}
                  (hits <code>https://api.anthropic.com...</code> directly, but
                  requires a CORS browser extension).
                </p>
              </div>
              ...
              >
                ...
                <p>
                  Because the Anthropic API blocks direct browser requests
                  (CORS), you must use a proxy when deploying this app.
                </p>
                <pre
                  style={{
              ...
              {inspectorLogs.map((log) => (
                ...
                >
                  ...
                  >
                    ...
                    <div>
                      Status:{" "}
                      <strong style={{ color: "#86EFAC" }}>{log.status}</strong>{" "}
                      · {log.durationMs}ms
                    </div>
                  </div>
                </div>
